...
- 最新ナンバリング公演以降に配信された新曲の予習プレイリスト作成
...
予習プレイリストは、終了済みの最新ナンバリング公演の日付を基準にします。管理画面の「予習プレイリスト」でシリーズを選ぶと、既存曲からSpotifyアーティスト候補を収集できます。取得済みのSpotify曲情報はブラウザへ保存するため、レート制限や再読み込みの後も未取得分だけ再開します。チェック済みアーティストだけをシリーズ別許可リストとして保存し、ローカル管理サーバーの利用中は`data/series-artists.json`へ自動反映します。新しく見つかった候補は既存の許可リストへ自動追加せず、手動確認後にチェックしてください。Spotify上の全作品を確認し、同じISRCのシングル版・アルバム再収録を同一音源としてまとめ、最初の配信日が基準日より後の曲だけを保存します。公開後は訪問ページからSpotify、Apple Music、Amazon Music向けに作成できます。
...
1. Spotifyへ接続する
2. 管理画面上部の「予習プレイリスト」を開く
3. シリーズを選び「候補をSpotifyから取得」を押す
4. 別シリーズのカバー元など、対象外のアーティストのチェックを外す
5. 「選択したアーティストから新曲を更新」を押す
6. 内容を確認して「全変更を公開」を押す

アーティストの選択結果と取得した曲は管理データへ保存されます。次回以降は保存済みの選択を使って新曲だけを更新できます。未来日付の公演や、ナンバリング公演フラグが付いていないイベントは基準日に使用しません。
