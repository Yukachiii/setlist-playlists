...
予習プレイリストは、終了済みの最新ナンバリング公演の日付を基準にします。管理画面の「予習プレイリスト」でシリーズを選ぶと、既存曲からSpotifyアーティスト候補を収集できます。チェック済みアーティストだけをシリーズ別許可リストとして保存し、ローカル管理サーバーの利用中は`data/series-artists.json`へ自動反映します。新しく見つかった候補は既存の許可リストへ自動追加せず、手動確認後にチェックしてください。Spotify上の全作品を確認し、同じISRCのシングル版・アルバム再収録を同一音源としてまとめ、最初の配信日が基準日より後の曲だけを保存します。公開後は訪問ページからSpotify、Apple Music、Amazon Music向けに作成できます。
