(function (root, factory) {
  ...
  function isSoundiizShareUrl(value) {
    return /^https:\/\/soundiiz\.com\/go\/import-playlist\/[A-Za-z0-9_-]+$/.test(String(value || ""));
  }
  ...
  function responseError(body, fallback, status) {
    const code = String(body?.code || `http_${status || "error"}`).trim();
  ...
  async function requestPlaylist({ eventPath, performanceId }, attempt = 0) {
    ...
    if (!response.ok) throw responseError(body, "プレイリストを作成できませんでした。", response.status);
